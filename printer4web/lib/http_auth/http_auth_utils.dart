...
String _computeHA1(String realm, String? algorithm, String username, String password, String? nonce, String? cnonce) {
...
Map<String, String?> computeResponse(String method, String path, String body, String? algorithm, String? qop, String? opaque, String realm, String? cnonce, String? nonce, int nc,
...
class DigestAuth {
  ...
  String getAuthString(String method, Uri url) {
    ...
    final authValues = computeResponse(method, path, '', _algorithm, _qop, _opaque, _realm!, _cnonce, _nonce, _nc, username, password);
    final authValuesString = authValues.entries.where((e) => e.value != null).map((e) => [e.key, '="', e.value, '"'].join('')).toList().join(', ');
...
AuthenticationScheme? pickSchemeFromAuthenticateHeader(String wwwAuthHeader) {
  final components = wwwAuthHeader.split(RegExp(r'[, ]')).where((s) => s.isNotEmpty).map((e) => e.toLowerCase()).toList();
